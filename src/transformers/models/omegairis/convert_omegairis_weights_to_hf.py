...
EPILOG_TXT = """Example:
    python transformers/src/transformers/models/omegairis/convert_omegairis_weights_to_hf.py --original_model_id model_path --output_hub_path org/omegairis
"""
...
def convert_omegairis_hub_to_hf(original_model_id, output_hub_path, push_to_hub):
    # The original model maps to AutoModelForCausalLM, converted we map to OmegaIrisForConditionalGeneration
    original_model = AutoModelForCausalLM.from_pretrained(original_model_id, trust_remote_code=True)
    # The original model doesn't use the omegairis processing objects
    ...
    with init_empty_weights():
        model = OmegaIrisForConditionalGeneration(config)
...
def main():
    ...
    convert_omegairis_hub_to_hf(args.original_model_id, args.output_hub_path, args.push_to_hub)
