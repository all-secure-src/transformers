...
class OmegaIrisProcessor(ProcessorMixin):
    r"""
    Constructs a OMEGAIRIS processor which wraps a LLama tokenizer and OMEGAIRIS image processor into a single processor.

    [`IdeficsProcessor`] offers all the functionalities of [`OmegaIrisImageProcessor`] and [`LlamaTokenizerFast`]. See
    the docstring of [`~IdeficsProcessor.__call__`] and [`~IdeficsProcessor.decode`] for more information.

    Args:
        image_processor (`OmegaIrisImageProcessor`):
            An instance of [`OmegaIrisImageProcessor`]. The image processor is a required input.
    ...
    image_processor_class = "OmegaIrisImageProcessor"
    ...
    def __call__(
        ...
        ```python
        >>> import requests
        >>> from transformers import OmegaIrisProcessor
        >>> from transformers.image_utils import load_image

        >>> processor = OmegaIrisProcessor.from_pretrained("model_path", image_seq_len=2)
    ...
    def apply_chat_template(
        ...
        **kwargs,
        ...
        """
        Overrides the tokenizer's `apply_chat_template` method to apply the OMEGAIRIS chat template by default
        if no chat template is provided.

        By default, the output isn't tokenized. This is because the OMEGAIRIS chat template is designed to insert
